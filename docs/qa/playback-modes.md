...
- **When** Controller가 `setRepeat { mode: 'all' }`
...
- **Given** 방의 controller
- **When** `setShuffle { shuffle: true }`
- **Then** ack `{ ok:true }`; `state.shuffle === true`

## MODE-03 — setRepeat/setShuffle은 Controller 전용
SPEC: §권한 — `setRepeat`/`setShuffle` 은 controller만.

- **Given** Player가 입장한 방
- **When** Player가 `setRepeat` 또는 `setShuffle` 발행
...
- **Given** Player + Controller, `setRepeat { mode:'one' }`, `changeTrack` A 적용
...
- **Given** Player + Controller, `setRepeat { mode:'all' }`, `changeTrack` A, `enqueueTrack` B
...
- **Given** 방의 controller, `setShuffle { shuffle:true }`, `enqueueTrack` B, `enqueueTrack` C
- **When** Controller가 `nextTrack {}`
...
- **Given** 방의 controller, `setRepeat { mode:'one' }`, `changeTrack` A, `enqueueTrack` B
- **When** Controller가 `nextTrack {}`
