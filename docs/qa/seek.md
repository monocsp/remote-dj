...
§탐색(Seek)/진행상황(`lastSeek`/`progress` 모델, seekTo Player 전용(메인) + 사유 선택,
...
- **When** Player(메인)가 `seekTo { seconds: 42 }` (사유 없음, 메인-전용)
...
## SEEK-05 — seekTo는 Player 전용(메인)
SPEC: §권한 — `seekTo` 는 메인-전용(Player만).

- **Given** Controller가 입장한 방
- **When** Controller(게스트)가 `seekTo { seconds: 10 }` 발행
