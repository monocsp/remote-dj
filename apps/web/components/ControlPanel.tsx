...
/**
 * Shared control surface for both roles. GUEST (리모컨) gets the controls the
 * server allows a guest to perform (changeTrack / enqueue / volume / play /
 * gain) plus a READ-ONLY queue + Activity Log. MAIN (the Player device) gets
...
export function ControlPanel({ variant }: { variant: 'guest' | 'main' }) {
  ...
  return (
    <>
      ...
      {/* schedule (예약) — main only */}
      {isMain && <ScheduleEditor schedule={schedule} onSave={actions.setSchedule} />}
