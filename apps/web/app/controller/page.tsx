...
import { type DayKey, type WeeklySchedule, isHHMM } from '@remote-dj/shared';
...
function ControllerInner() {
  ...
  // Local draft of the weekly schedule, committed by the 저장 button (not per
  // keystroke). Synced from authoritative state; falls back to the office default.
      ...
      <section className="rounded-xl bg-neutral-900 p-4">
        ...
        <button
          ...
          onClick={() => {
            ...
            if (invalid) {
              setScheduleHint('켜진 요일은 시작 < 종료여야 합니다 (HH:MM)');
              return;
            }
            setScheduleHint(null);
            void actions.setSchedule(draft).then((ack) => {
              if (!ack.ok) setScheduleHint(ack.error ?? '저장 실패');
            });
          }}
        ...
        {scheduleHint && <p className="mt-2 text-xs text-red-400">{scheduleHint}</p>}
